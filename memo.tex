...
\section{Navigating the Filesystem}
    ...
    \CommandEntry{ls -sh}
    {
        Display file's size in human-readable format.
    }

    \CommandEntry{ls -g}
    {
        Only display file's default group in the long listing format.
    }

    \CommandEntry{ls -o}
    {
        Only display file's owner in the long listing format.
    }
...
\section{Benchmark}
        ...
        \CommandEntry{sysbench memory [options] run}
        {
            Run memory benchmark. Common \texttt{options} are \\
            \texttt{--memory-block-size} \\
            \texttt{--memory-total-size} \\
            \texttt{--memory-oper=[ read | write ]} \\
            \texttt{--memory-access-mode=[ seq | rnd ]}
        }
...
\section{Permission Management}
    ...
    \subsection{Group Management}
        ...
        \CommandEntry{group}
        {
            List all names of groups that the user belongs to.
        }
...
\section{Resource Monitoring}
    ...
    \CommandEntry{sudo intel\_gpu\_top}
    {
        Task-manager-like monitor for Intel GPU
    }

    \CommandEntry{watch -n1 -d nvidia-smi}
    {
        Task-manager-like monitor for NVIDIA GPU       
    }
...
\section{Git}
    ...
    \CommandEntry{git add \$(git ls-files -o --exclude-standard)}
    {
        Add all \textbf{untracked} files to the staging area.
    }
...
\section{GPG}
    ...
    \CommandEntry{gpg --listkeys}
    {
        List all imported keys.
    }

    \CommandEntry{gpg --recv-keys <keyid>}
    {
        Import the pubkey whose issuer id is \texttt{<keyid>}
    }

    \CommandEntry{gpg --delete-keys <keyid>}
    {
        Delete the imported pubkey whose issuer id is \texttt{<keyid>} 
    }

    \CommandEntry{gpg --verify <sigfile> <file>}
    {
        Verify the authenticity of \texttt{<file>} with \texttt{<sigfile>}
    }

    \CommandEntry{gpg --list-packets <param>}
    {
        Print summary of \texttt{<param>}, which can be either a key issuer id 
        or signature file. \\
        This command is very useful for extracting key issuer ID when only given 
        a signature file.
    }
...
\section{Misc}
    ...
    \CommandEntry{date}
    {
        ...
        \texttt{date -d "<some\_date>" +\%s}: get the epoch time for \texttt{<some\_date>}. \\ e.g. \texttt{date -d "Jan 1, 2022" +\%s}
    }
...
\section{Shell Scripting}
    ...
    \ScriptEntry{\texttt{if-then} Statement}
    {
        Format: \\
        \texttt{if <command>} \\
        \texttt{then} \\
        ...
        \texttt{fi};
        Alternative format: \\
        \texttt{if <command>; then} \\
        \mbox{\texttt{~~~<commands>}} \\
        \texttt{fi};
        Rule: if \texttt{<command>} returns 0, \texttt{<commands>} will be executed, otherwise will be skipped.;
        \texttt{if} can only evaluate the return status of \texttt{<command>}
    }

    \ScriptEntry{\texttt{if-then-else} Statement}
    {
        Format: \\
        \texttt{if <command>} \\
        \texttt{then} \\
        ...
        \mbox{\texttt{~~~<commands>}} \\
        \texttt{fi};
        Rule: if \texttt{<command>} returns 0, \texttt{then} block will be executed, otherwise \texttt{else} block will be executed.;
        \texttt{if-then} statement can be nested. If nested in \texttt{else} block, \\
        \texttt{if <command>} \\
        \texttt{then} \\
        \mbox{\texttt{~~~<commands>}} \\
        \texttt{elif <command>} \\
        ...
        \mbox{\texttt{~~~<commands>}} \\
        \texttt{fi};
        \texttt{else} block belongs to the \texttt{if} / \texttt{elif} block directly precede it.;
        \texttt{if} statements are executed in order, only the first one that has a 0 return status will be executed.
    }

    \ScriptEntry{\texttt{test} Command}
    {
        Format: \texttt{test <condition>} or \texttt{[ <condition> ]};
        For brackets form, you must have a space after the first bracket and a space before the second bracket.;
        \texttt{[ <condition1> ] \&\& [ <condition2> ]}, \texttt{[ <condition1> ] || [ <condition2> ]}. They are short-circuit.;
        If \texttt{<condition>} is true, \texttt{test} returns 0, otherwise a non-zero status code will be returned.;
        If \texttt{<condition>} is empty, it will be evaluated to be false.;
        Combine with \texttt{if} statement: \\
        \texttt{if test <condition>} \\
        \texttt{then} \\
        \mbox{\texttt{~~~<commands>}} \\
        \texttt{fi} \\ \\
        or \\ \\
        ...
        Usage:
        \begin{itemize}
            \item numeric comparison (can only handle integers)
            \begin{itemize}
                \item \texttt{<n1> -eq <n2>}: equal 
                \item \texttt{<n1> -ne <n2>}: not equal 
                \item \texttt{<n1> -lt <n2>}: less than 
                \item \texttt{<n1> -gt <n2>}: greater than
                \item \texttt{<n1> -le <n2>}: less than or equal to
                \item \texttt{<n1> -ge <n2>}: greater than or equal to
            \end{itemize}
            \item string comparison (it's better to enclose strings with quotation marks)
            \begin{itemize}
                \item \texttt{<str1> = <str2>}: equal
                \item \texttt{<str1> != <str2>}: not equal
                \item \texttt{<str1> < <str2>}: less than ($<$ must be escaped, or will be treated as input redirection. Opposite to \texttt{sort} order)
                \item \texttt{<str1> > <str2>}: greater than ($>$ must be escaped, or will be treated as output redirection. Opposite to \texttt{sort} order)
                \item \texttt{-n <str>}: is not empty, equivalent to \texttt{test <str>}. if a variable is never defined, it will be treated as empty. It's a good idea to test any strings before comparing them.
                \item \texttt{-z <str>}: is empty (zero length)
            \end{itemize}
            \item file comparison
            \begin{itemize}
                \item \texttt{-e <file>}: exist
                \item \texttt{-f <file>}: exist and is a file
                \item \texttt{-d <file>}: exist and is a directory (good thing to do before writing to or jumping to a directory)
                \item \texttt{-r <file>}: exist and is readable
                \item \texttt{-w <file>}: exist and is writable
                \item \texttt{-x <file>}: exist and is executable
                \item \texttt{-s <file>}: exist and is not empty
                \item \texttt{-O <file>}: exist and is owned by the current user
                \item \texttt{-G <file>}: exist and the \textbf{default group} is the same as the current user
                \item \texttt{<file1> -ot <file2>}: older than (meta-info change time or modification time)
                \item \texttt{<file1> -nt <file2>}: newer than (meta-info change time or modification time). Both \texttt{-ot} and \texttt{-nt} won't check the existence of the files. \textbf{So it's imperative to check the existence.}
            \end{itemize}
        \end{itemize}
    }
