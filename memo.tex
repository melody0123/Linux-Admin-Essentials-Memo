...
\section{Networks}
    ...
    {
        Display the established socket whose [ remote | local ] port number is \texttt{<port \#>}.;
        \texttt{dport} is the remote port. \texttt{sport} is the local port.;
        Filters can be connected using \texttt{and}, \texttt{or}. For example, \texttt{ss -o state established '( dport = :<port \#1> or sport = :<port \#2> )'}
    }
